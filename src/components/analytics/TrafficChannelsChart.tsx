...
export default function TrafficChannelsChart({ days }: { days: RangeDays }) {
  const { data, loading, error } = useAnalyticsReport<Channels>("channels", days);

  if (error) return <AnalyticsErrorCard error={error} title="Traffic Channels" />;
