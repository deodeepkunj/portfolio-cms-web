...
import type { AdGroups } from "@/lib/googleAnalytics";
...
export default function AdGroupsTable({ data, days }: Props) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Ad Groups</h3>
        <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
          Performance by ad group in the last {days === 1 ? "day" : days === "all" ? 365 : days} days
        </p>
      </div>
      <div className="max-w-full overflow-x-auto">
        <Table>
          <TableHeader className="border-y border-gray-100 dark:border-gray-800">
            <TableRow>
              <TableCell isHeader className="py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Ad Group</TableCell>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
            {rows.length === 0 ? (
              <TableRow>
                <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400" colSpan={7}>
                  No ad group data found. Link your GA4 property to Google Ads in GA Admin → Google Ads Links.
                </TableCell>
              </TableRow>
            ) : (
              rows.map((row, index) => (
                <TableRow key={`${row.adGroup}-${index}`}>
                  <TableCell className="py-3">
                    <p className="font-medium text-gray-800 text-theme-sm dark:text-white/90">{row.adGroup}</p>
                  </TableCell>
